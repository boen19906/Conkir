...
function hasLOS(gs: GameState, x1: number, y1: number, x2: number, y2: number): boolean {
  const dx = x2 - x1, dy = y2 - y1;
  const steps = Math.ceil(Math.hypot(dx, dy));
  for (let i = 1; i < steps; i++) {
    if (isL(gs, (x1 + dx * i / steps) | 0, (y1 + dy * i / steps) | 0)) return false;
  }
  return true;
}
...
export function updUnits(gs: GameState) {
  ...
  for (const s of gs.unt) {
    if (s.ty === 't') {
      ...
      const tgtFriendly = tgtOwner === s.ow || (tgtOwner >= 0 && gD(gs, s.ow, tgtOwner) === 'peace');
      if (distToTarget < 15 && tgtFriendly) { gs.P[s.ow].troops += s.tr || 0; s.hp = 0; continue; }
    ...
    for (const o of gs.unt) {
      ...
      if (d < nd && hasLOS(gs, s.x, s.y, o.x, o.y)) { nd = d; ne = o; }
    }
    ...
    let chaseTarget: typeof gs.unt[0] | null = null, chaseDist = 1e9;
    for (const tr of gs.unt) {
    ...
    for (let ti = gs.unt.length - 1; ti >= 0; ti--) {
      ...
      if (Math.hypot(tr.x - s.x, tr.y - s.y) < 16) {
        ...
        const sizeMult = Math.max(0.25, Math.sqrt(Math.max(1, gs.P[tr.ow]?.territory || 1) * Math.max(1, dstPortOwner >= 0 ? (gs.P[dstPortOwner]?.territory || 1) : 1)) / 50000);
        ...
        gs.addNotif(tr.ow, `🏴‍☠️ ${gs.P[s.ow]?.name} captured your trade ship! -$${Math.round(gold)}`, '#E74C3C');
        gs.unt.splice(ti, 1);
      }
    }
