...
function openFrontPriority(gs: GameState, ni: number, pi: number, tickNow: number) {
  ...
  for (const [dx, dy] of [[-1, 0], [1, 0], [0, -1], [0, 1]]) {
    ...
    if (B(nx, ny) && gs.own[I(nx, ny)] === pi) numOwnedByMe++;
  }
  const noise = (Math.random() * 8) | 0;
  return (noise + 10) * (1 - numOwnedByMe * 0.5) + tickNow;
}

export function waveCanClaim(gs: GameState, pi: number, tileOwner: number, targetOwner: number | null) {
  if (tileOwner === pi) return false;
  if (targetOwner === null) {
    if (tileOwner >= 0 && gD(gs, pi, tileOwner) === 'peace') return false;
    return true;
  }
...
export function mkWave(gs: GameState, pi: number, cx: number, cy: number, tr: number, targetOwner: number | null) {
  const p = gs.P[pi]; if (!p || !p.alive || tr < 1) return;
  ...
  for (let y = Math.max(0, cy - sr); y < Math.min(H, cy + sr); y++)
    for (let x = Math.max(0, cx - sr); x < Math.min(W, cx + sr); x++) {
      ...
      for (const [dx, dy] of [[-1, 0], [1, 0], [0, -1], [0, 1]]) {
        ...
        if (!B(nx, ny)) continue;
        const ni = I(nx, ny);
        if (gs.ter[ni] === 0) continue;
        const no = gs.own[ni];
        if (no === pi) continue;
        if (!waveCanClaim(gs, pi, no, targetOwner)) continue;
        if (!inHeap.has(ni)) { inHeap.add(ni); heap.enqueue(ni, openFrontPriority(gs, ni, pi, gs.tk)); }
      }
    }
  ...
  for (const w of gs.wav) {
    ...
    if (overlap) {
      w.troops += a;
      for (const ni of inHeap) if (!w.inHeap.has(ni)) { w.inHeap.add(ni); w.heap.enqueue(ni, openFrontPriority(gs, ni, pi, gs.tk)); }
      return;
    }
  }

  gs.wav.push({ pi, troops: a, heap, inHeap, id: gs.nextWid(), targetOwner, _pressing: new Set() });
}
...
export function procWaves(gs: GameState) {
  ...
  gs.wav = gs.wav.filter(w => w.troops > 0 && w.heap.size() > 0 && gs.P[w.pi]?.alive);

  const tpt = C.tilesPerTick;
  for (let wi = gs.wav.length - 1; wi >= 0; wi--) {
    const w = gs.wav[wi];
    ...
    while (w.heap.size() > 0 && cl < tpt && w.troops > 0) {
      ...
      if (gs.ter[ni] === 0) continue;
      const tileOwner = gs.own[ni];
      if (tileOwner === w.pi) continue;
      if (!waveCanClaim(gs, w.pi, tileOwner, w.targetOwner)) continue;

      const tx = ni % W, ty = (ni / W) | 0;
      let onBorder = false;
      for (const [dx, dy] of [[-1, 0], [1, 0], [0, -1], [0, 1]]) {
        if (B(tx + dx, ty + dy) && gs.own[I(tx + dx, ty + dy)] === w.pi) { onBorder = true; break; }
      }
      if (!onBorder) continue;

      let fortMult = 1.0, fortCapping = false;
      if (tileOwner >= 0) {
        for (const fb of gs.bld) {
      ...
      if (tileOwner >= 0) {
        const def = gs.P[tileOwner];
        if (def?.alive && def.territory > 0) {
          const density = def.troops / def.territory;
          const debuff = getDefenseMultiplier(gs, tileOwner);
      ...
      for (const b of gs.bld) {
        ...
        if (bnames[b.type]) gs.addNotif(w.pi, `Captured ${bnames[b.type]}!`, '#2ECC71');
      }
      cl++;
      ...
      for (const [dx, dy] of [[-1, 0], [1, 0], [0, -1], [0, 1]]) {
        ...
        const o2 = gs.own[nni];
        if (o2 === w.pi) continue;
        if (!waveCanClaim(gs, w.pi, o2, w.targetOwner)) continue;
        if (w.inHeap.has(nni)) continue;
        w.inHeap.add(nni);
        w.heap.enqueue(nni, openFrontPriority(gs, nni, w.pi, gs.tk));
      }
    }

    if (w.troops <= 0 || w.heap.size() === 0) {
      if (w.troops > 0 && gs.P[w.pi]?.alive) gs.P[w.pi].troops += w.troops;
      gs.wav.splice(wi, 1);
    }
  }
}
