  ...
  for (const s of unt) {
    if (s.ty === 't') {
      ...
      if (!s.path || s.path.length === 0) {
        s.stuck = (s.stuck || 0) + 1;
        if ((s.stuck || 0) > 200) { P[s.ow].troops += s.tr || 0; s.hp = 0; }
        continue;
      }
      const ni = s.path[0];
    ...
    let ne: typeof unt[0] | null = null, nd = 1e9;
    for (const o of unt) {
    ...
    for (let ti = unt.length - 1; ti >= 0; ti--) {
      ...
      if (Math.hypot(tr.x - s.x, tr.y - s.y) < 16) {
        ...
        const sizeMult = Math.max(0.25, Math.sqrt(Math.max(1, P[tr.ow]?.territory || 1) * Math.max(1, dstPortOwner >= 0 ? (P[dstPortOwner]?.territory || 1) : 1)) / 25000);
    ...
    if (!moved) {
      ...
      if (sepX !== 0 || sepY !== 0) {
        ...
        if (isW(nx | 0, ny | 0)) { s.x = nx; s.y = ny; s.stuck = 0; }
      }
      if ((s.stuck || 0) > 120) {
        for (let r = 1; r < 20; r++) for (const [dx, dy] of [[-r, 0], [r, 0], [0, -r], [0, r]]) {
          const nx = s.x + dx, ny = s.y + dy;
          if (isDeepW(nx | 0, ny | 0)) { s.x = nx; s.y = ny; s.stuck = 0; r = 99; break; }
        }
      }
    }
  }
  // filter dead units in-place by marking hp=0 then splicing
  for (let i = unt.length - 1; i >= 0; i--) { if (unt[i].hp <= 0) unt.splice(i, 1); }
}

export function updBullets() {
